...
test("Pyth decimal and confidence conversion is deterministic", () => {
  ...
  assert.ok(Math.abs(decimalPrice(feed) - 340) < 1e-9);
  assert.equal(confidenceBps(feed), 1);
});

test("tracking error is expressed in basis points", () => {
  assert.ok(Math.abs(trackingErrorBps(339.83, 340) - 5) < 1e-9);
  assert.ok(Math.abs(trackingErrorBps(343.4, 340) - 100) < 1e-9);
});
...
test("signed Pyth response becomes evidence-bearing market records", () => {
  ...
  assert.ok(Math.abs(parsed.feeds[0].price - 340) < 1e-9);
  assert.ok(Math.abs(parsed.feeds[1].price - 339.83) < 1e-9);
  ...
  assert.ok(Math.abs(market.trackingErrorBps.value - 5) < 1e-9);
